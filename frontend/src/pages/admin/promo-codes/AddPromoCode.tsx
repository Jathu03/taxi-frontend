...
// You likely have a vehicle service to get classes. Importing hypothetical service.
// If you don't have this, I'll add a helper function below.
...
export default function AddPromoCode() {
  ...
  const [vehicleClasses, setVehicleClasses] = useState<{ id: number; className: string }[]>([]);

  // Form State
  const [formData, setFormData] = useState({
    code: "",
    description: "",
    vehicleClassIds: [] as number[], // Storing IDs instead of names
    discountIn: "PERCENTAGE", // Matches Backend Enum
    discountValue: "0",
    startDate: "",
    endDate: "",
    isActive: true,
  ...
  useEffect(() => {
    const fetchVehicleClasses = async () => {
      try {
        // Calling Vehicle Service via Gateway
        // Adjust endpoint based on your actual Vehicle Service API
        const response = await axiosClient.get("/vehicle-service/vehicle-classes/active"); 
        if (response.data && response.data.success) {
          // Mapping response to simple object
          const classes = response.data.data.map((vc: any) => ({
            id: vc.id,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        discountType: formData.discountIn, // "PERCENTAGE" or "FIXED"
        ...
        name: formData.code, // Using code as name if not provided separately
        minimumFare: 0,
        applicableTo: "ALL", 
      };
