...
export default function TukPassengerOnboard() {
  ...
  const loadOnboard = async () => {
    try {
      ...
      // ✅ PASSENGER_ONBOARD + ✅ ONLY TUK
      const onboardTuk = (data || []).filter(
        (b) =>
          b.status === BookingStatus.PASSENGER_ONBOARD &&
          Number(b.vehicleClassId) === TUK_CLASS_ID
      );

      setRows(onboardTuk.map(mapBookingToRow));
    } catch (error) {
      console.error(error);
      toast.error("Failed to load TUK onboard bookings");
      setRows([]);
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="p-6 space-y-6 bg-gradient-to-br from-white via-orange-50/30 to-yellow-50/30 min-h-screen">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-orange-700">TUK Passenger Onboard</h1>
          <p className="text-muted-foreground mt-1">TUK hires currently in progress</p>
        </div>
        <Badge className="text-lg px-4 py-2 bg-orange-600">
          {filteredData.length} On Board (TUK)
        </Badge>
      </div>
