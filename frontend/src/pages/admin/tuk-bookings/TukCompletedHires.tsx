...
const columns = (navigate: ReturnType<typeof useNavigate>): ColumnDef<TukCompletedHire>[] => [
...
const mockTukCompleted: TukCompletedHire[] = [
  { id: "1", bookingId: "TUK-CMP-001", customerName: "Sarah Johnson", driverName: "Sunil Perera", vehicleNumber: "WP-TUK-1234", fare: "LKR 450", completionTime: "2024-03-20 11:30", status: "Completed" },
];
...
export default function TukCompletedHires() {
  ...
  const {
    ...
    initialData: mockTukCompleted,
  });

  const filteredData = useMemo(() => {
    return data.filter((booking) => {
      if (!filterText) return true;
      const value = booking[filterBy as keyof TukCompletedHire]?.toString().toLowerCase() || "";
  ...
  return (
    <div className="p-6 space-y-6 bg-gradient-to-br from-white via-purple-50/30 to-blue-50/30 min-h-screen">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-[#6330B8]">TUK Completed Hires</h1>
          <p className="text-muted-foreground mt-1">History of all successfully completed TUK bookings</p>
        </div>
