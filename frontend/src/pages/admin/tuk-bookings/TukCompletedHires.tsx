...
export default function TukCompletedHires() {
  ...
  const loadCompleted = async () => {
    try {
      ...
      // ✅ COMPLETED + ✅ ONLY TUK
      const completedTuk = (data || []).filter(
        (b) => b.status === BookingStatus.COMPLETED && Number(b.vehicleClassId) === TUK_CLASS_ID
      );

      setRows(completedTuk.map(mapBookingToRow));
    } catch (error) {
      console.error(error);
      toast.error("Failed to load TUK completed hires");
      setRows([]);
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="p-6 space-y-6 bg-gradient-to-br from-white via-orange-50/30 to-yellow-50/30 min-h-screen">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-orange-700">TUK Completed Hires</h1>
          <p className="text-muted-foreground mt-1">
            Completed bookings for TUK only
          </p>
        </div>
        <Badge className="text-lg px-4 py-2 bg-orange-600">
          {filteredData.length} Completed (TUK)
        </Badge>
      </div>
