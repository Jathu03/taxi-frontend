// src/pages/admin/bookings/TukCancelledHires.tsx
"use client";
...
export default function TukCancelledHires() {
  ...
  useEffect(() => {
    const debounce = setTimeout(() => fetchCancelledHires(), 500);
    return () => clearTimeout(debounce);
  }, [fetchCancelledHires]);

  // Filter locally + ONLY TUK
  ...
  return (
    <div className="p-6 space-y-6 bg-gradient-to-br from-white via-orange-50/30 to-yellow-50/30 min-h-screen">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-orange-700">TUK Cancelled Hires</h1>
          <p className="text-muted-foreground mt-1">
            History of cancelled TUK bookings and reasons
          </p>
        </div>
        <Badge className="text-lg px-4 py-2 bg-orange-600 hover:bg-orange-700">
          {filteredData.length} Cancelled (TUK)
        </Badge>
      </div>
