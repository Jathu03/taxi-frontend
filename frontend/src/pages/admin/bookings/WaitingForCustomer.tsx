        ...
        <div className="flex justify-end gap-2">
          <Button
            ...
            onClick={async () => {
              try {
                ...
                toast.success("Trip started! Passenger is onboard.");
                refresh();
              } catch (error) {
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem
                onClick={() => window.open(`tel:${row.original.phoneNumber}`)}
              >
...
export default function WaitingForCustomer() {
  ...
  const loadWaitingBookings = async () => {
    try {
      ...
      // ✅ WAITING_FOR_CUSTOMER + ✅ EXCLUDE TUK
      const waitingNonTuk = (data || []).filter(
        (b) =>
          b.status === BookingStatus.WAITING_FOR_CUSTOMER &&
          Number(b.vehicleClassId) !== TUK_CLASS_ID
      );

      setRows(waitingNonTuk.map(mapBookingToRow));
    } catch (error) {
      console.error(error);
      toast.error("Failed to load waiting bookings");
      setRows([]);
    } finally {
      setIsLoading(false);
    }
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-[#6330B8]">
            Waiting for Customer (Excluding TUK)
          </h1>
          <p className="text-muted-foreground mt-1">
            Drivers waiting at pickup location
          </p>
        </div>
