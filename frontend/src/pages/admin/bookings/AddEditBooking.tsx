  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload: CreateBookingRequest = {
        ...
        corporateId: formData.isCorporate && formData.corporateId ? parseInt(formData.corporateId) : undefined,
  ...
  return (
    <div className="p-6 space-y-6 bg-gradient-to-br from-white via-purple-50/30 to-blue-50/30 min-h-screen">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            {isEditMode && <p className="text-sm text-gray-500">Booking ID: {passedBooking?.bookingId || id}</p>}
          </div>
        </div>
        <Button onClick={handleSubmit} className="bg-[#6330B8] hover:bg-[#5028a0]" disabled={isLoading}>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <CardContent className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            {formData.isCorporate && (
              <div className="space-y-2 animate-in fade-in">
                <Label className="flex items-center gap-2"><Building2 className="h-4 w-4" /> Corporate ID</Label>
                <Input 
                  type="number"
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-3 pt-2 border-t border-dashed">
              <div className="flex items-center justify-between">
                ...
                <Button type="button" variant="outline" size="sm" onClick={addStop} className="h-7 text-xs gap-1">
              ...
              {formData.stops.map((stop, index) => (
                <div key={index} className="flex items-center gap-2">
                  ...
                  />
                  <Button type="button" variant="ghost" size="icon" onClick={() => removeStop(index)} className="text-red-400">
                    <X className="h-4 w-4" />
                  </Button>
                </div>
              ))}
        ...
        {/* Vehicle & Options */}
        <Card>
          <CardHeader><CardTitle>Vehicle & Options</CardTitle></CardHeader>
          <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-4">
             <div className="space-y-2">
              <Label>Vehicle Class</Label>
              <Select value={formData.vehicleClassId} onValueChange={(v) => handleChange("vehicleClassId", v)}>
                <SelectTrigger><SelectValue placeholder="Select class" /></SelectTrigger>
                <SelectContent>
                  {vehicleClasses.map((vc) => (
                    <SelectItem key={vc.id} value={vc.id.toString()}>{vc.className}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Select value={formData.paymentType} onValueChange={(v) => handleChange("paymentType", v)}>
                ...
                <SelectContent>
                  ...
                  {formData.isCorporate && <SelectItem value="CORPORATE_BILLING">Corporate Billing</SelectItem>}
                </SelectContent>
              </Select>
            </div>
