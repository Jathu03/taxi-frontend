...
export default function EnrouteBookings() {
  ...
  const loadEnroute = async () => {
    try {
      ...
      // ✅ ENROUTE + ✅ EXCLUDE TUK
      const enrouteNonTuk = (data || []).filter(
        (b) =>
          b.status === BookingStatus.ENROUTE &&
          Number(b.vehicleClassId) !== TUK_CLASS_ID
      );

      setRows(enrouteNonTuk.map(mapBookingToRow));
    } catch (error) {
      console.error(error);
      toast.error("Failed to load en route bookings");
      setRows([]);
    } finally {
      setIsLoading(false);
    }
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-[#6330B8]">
            En Route Bookings (Excluding TUK)
          </h1>
          <p className="text-muted-foreground mt-1">
            Drivers currently on their way to pickup
          </p>
        </div>
        <Badge className="text-lg px-4 py-2 bg-orange-600">
          {filteredData.length} En Route
        </Badge>
      </div>
