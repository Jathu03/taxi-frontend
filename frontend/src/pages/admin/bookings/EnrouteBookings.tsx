...
const columns = (navigate: ReturnType<typeof useNavigate>): ColumnDef<EnrouteBooking>[] => [
  ...
  { accessorKey: "status", header: () => <span className="font-bold text-black">Status</span>, cell: ({ row }) => <Badge className="bg-orange-100 text-orange-700 border-orange-200">{row.original.status}</Badge> },
  {
    id: "actions",
...
const mockEnrouteBookings: EnrouteBooking[] = [
  { id: "1", bookingId: "ENR-001", customerName: "Sarah Johnson", driverName: "Sunil Perera", vehicleNumber: "WP-CAB-1234", pickupLocation: "Colombo 03", estimatedArrival: "5 mins", status: "En Route" },
  { id: "2", bookingId: "ENR-002", customerName: "Michael Silva", driverName: "Kamal Fernando", vehicleNumber: "WP-CAB-5678", pickupLocation: "Dehiwala", estimatedArrival: "12 mins", status: "En Route" },
];
...
export default function EnrouteBookings() {
  ...
  const filteredData = useMemo(() => {
    return data.filter((booking) => {
      ...
      const value = booking[filterBy as keyof EnrouteBooking]?.toString().toLowerCase() || "";
  ...
  return (
    <div className="p-6 space-y-6 bg-gradient-to-br from-white via-purple-50/30 to-blue-50/30 min-h-screen">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-[#6330B8]">En Route Bookings</h1>
          <p className="text-muted-foreground mt-1">Drivers currently on their way to pickup</p>
        </div>
        <Badge className="text-lg px-4 py-2 bg-orange-600">{data.length} En Route</Badge>
      </div>
