...
export default function AppPendingBookings() {
  ...
  const filteredData = useMemo(() => {
    return data.filter((booking) => {
      if (filterText) {
        const value = booking[filterBy as keyof AppPendingBooking]?.toString().toLowerCase() || "";
        if (!value.includes(filterText.toLowerCase())) return false;
      }
      if (platform !== "all" && booking.appPlatform !== platform) return false;
      if (vehicleType !== "all" && booking.vehicleType !== vehicleType) return false;
      return true;
    });
  }, [data, filterText, filterBy, platform, vehicleType]);
