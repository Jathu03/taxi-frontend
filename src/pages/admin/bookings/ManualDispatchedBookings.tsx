...
import { Eye, CheckCircle, XCircle, RefreshCw, RotateCcw } from "lucide-react";
...
const columns = (navigate: ReturnType<typeof useNavigate>): ColumnDef<ManualDispatchedBooking>[] => [
  {
    ...
    header: () => <span className="font-bold text-black text-center block">Booking</span>,
    cell: ({ row }) => (
  ...
  { accessorKey: "organization", header: () => <span className="font-bold text-black">Org</span> },
  {
  ...
  { accessorKey: "pickupAddress", header: () => <span className="font-bold text-black max-w-[200px] truncate block">Pickup Address</span> },
...
const mockManualDispatchedBookings: ManualDispatchedBooking[] = [
  { id: "1", bookingId: "295749", organization: "BCD Travels", customerName: "BCD Travel", passengerNumber: "0714788999", hireType: "On The Meter", pickupTime: "12/04/2025 15:30", pickupAddress: "Colombo 03", bookedBy: "devinda95", dispatchedBy: "devinda95", dispatchedTime: "12/04/2025 15:00", testBooking: "", driverName: "4124 Tharanga", vehicleNumber: "CBO-4831", fareScheme: "Mileage Calculator" },
  { id: "2", bookingId: "295745", organization: "BCD Travels", customerName: "BCD Travel", passengerNumber: "0714788999", hireType: "On The Meter", pickupTime: "12/04/2025 15:30", pickupAddress: "Colombo 07", bookedBy: "devinda95", dispatchedBy: "devinda95", dispatchedTime: "12/04/2025 14:48", testBooking: "", driverName: "361 Amara", vehicleNumber: "PE-3912", fareScheme: "STANDARD" },
];
...
export default function ManualDispatchedBookings() {
  ...
  const filteredData = useMemo(() => {
    return data.filter((item) => {
      if (filterText) {
        const value = item[filterBy as keyof ManualDispatchedBooking]?.toString().toLowerCase() || "";
  ...
  return (
    <div className="p-6 space-y-6 bg-gradient-to-br from-white via-purple-50/30 to-blue-50/30 min-h-screen">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-[#6330B8]">Manually Dispatched</h1>
          <p className="text-muted-foreground mt-1">Manage bookings that were manually assigned to drivers</p>
        </div>
        <Badge className="text-lg px-4 py-2 bg-[#6330B8]">{data.length} Active</Badge>
      </div>
